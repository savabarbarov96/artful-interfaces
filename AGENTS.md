...
- `/mvp-development` (`src/pages/MvpDevelopmentLanding.tsx`, SaaS/MVP page from the same report) shipped the first non-subscription pricing shape: `src/components/landing/MvpPricing.tsx` — one-time fixed packages (styled like `Pricing.tsx`'s cards) plus an optional monthly care/iteration add-on. The euro figures in it are explicitly marked in-code as indicative/unconfirmed by the owner. Any future page needing "fixed one-time" or "setup+monthly" pricing (AI agent dev, security testing, compliance) should follow this pattern rather than reusing `Pricing.tsx`.
