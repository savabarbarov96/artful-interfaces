...
- `/compliance` (`src/pages/ComplianceLanding.tsx`, shipped 2026-07) covers NIS2/ISO 27001/GDPR readiness — explicitly *preparation*, not certificate issuance (only accredited bodies like TÜV NORD/LRQA/Bureau Veritas can certify; the page says so in its first FAQ). It does not use the shared `Pricing.tsx` (99/299 € plans) — it has its own `src/components/sections/CompliancePricing.tsx` with a hybrid shape (one-time readiness project + monthly compliance-maintenance subscription); those numbers are marked indicative in a code comment and need owner confirmation before being treated as final. The report's #5 "security testing" candidate page was explicitly declined by the owner — do not add pentest/security-audit content to this page or elsewhere without a separate decision.
